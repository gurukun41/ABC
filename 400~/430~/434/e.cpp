...
int main(){
	...
	for (int i = 0; i <= 2000; i++) {
		for (int j = 1; j <= 2000; j++) A[i][j] += A[i][j - 1];
	}
	for (int i = 1; i <= 2000; i++) {
		for (int j = 0; j <= 2000; j++) A[i][j] += A[i - 1][j];
	}
    ll sum = 0;
    rep(i,0,2000){
        rep(j,0,2000){
    ...
    rep(i,0,2000){
        rep(j,0,i){
            ...
            if(i-1>=0&&j-1>=0){
                sum1[j][i] += sum1[j-1][i] + sum1[j][i-1] - sum1[j-1][i-1];
            } else if(j-1>=0){
                sum1[j][i] += sum1[j-1][i];
            } else if(i-1>=0){
                sum1[j][i] += sum1[j][i-1];
        ...
        if(i-1>=0){
            sum1[i][i] += sum1[i-1][i] + sum1[i][i-1] - sum1[i-1][i-1];
        }
    ...
    rep(i,0,N){
        ll ans = sum;
        ans += sum1[D[i]-1][R[i]-1];
        if(L[i]-1 >= 0 && U[i]-1>=0){
            ans -= sum1[U[i]-1][R[i]-1] + sum1[D[i]-1][L[i]-1] - sum1[U[i]-1][L[i]-1];
        } else if(L[i] -1 >=0){
            ans -= sum1[D[i]-1][L[i]-1];
        } else if(U[i] -1 >=0){
            ans -= sum1[U[i]-1][R[i]-1];
        }
        cout << ans << "\n";
    }
}