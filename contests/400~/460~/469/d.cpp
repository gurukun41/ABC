...
int main() {
    ...
    scan(X);
    set<pl> c;
    c.insert({X[0].first, -1});
    c.insert({X[0].second, -1});
    c.insert({X[0]});
    ...
    rep(i, 1, M) {
        ...
        for (pl p : c) {
            if (p.second == -1) {
                if (p.first != pt.first) {
                    if (p.first != pt.second) {
                        nex.erase(p);
                        if (p.first < pt.second) {
                            nex.insert({p.first, pt.second});
                        } else {
                            nex.insert({pt.second, p.first});
                        }
                        if (p.first < pt.first) {
                            nex.insert({p.first, pt.first});
                        } else {
                            nex.insert({pt.first, p.first});
                        }
                    }
                }
            } else {
                if ((p.first != pt.first) &&(p.first != pt.second)&&(p.second != pt.first)&&(p.second != pt.second)) {
                    nex.erase(p);
                }
            }
