...
int main(){
    vvl A(3,vl(6));rep(i,0,3) rep(j,0,6) cin >> A[i][j];
    vvl B(3,vl(3,0));
    rep(i,0,3){
        rep(j,0,6){
            if(A[i][j]-4 >= 0){
    ...
    ld ans = 0.0;
    rep(i,0,3){
        ans += (ld)(B[0][i%3])*(ld)(B[1][(i+1)%3])*(ld)(B[2][(i+2)%3])*1.0;
        ans += (ld)(B[0][i%3])*(ld)(B[1][(i+2)%3])*(ld)(B[2][(i+1)%3])*1.0;
    }

    cout <<  std::fixed << std::setprecision(15) << ans/(6.0*6.0*6.0)  << "\n";
}