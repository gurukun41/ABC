...
struct LazySegmentTree_Max {
   ...
   public:
    ...
    LazySegmentTree_Max(ll n) : LazySegmentTree_Max(vl(n, INF)) {}
...
int main(){
    ...
    LazySegmentTree_Min mT(A);
    LazySegmentTree_Sum ST(A);
    LazySegmentTree_Max MT(A);
    vl out(0);
    rep(_,0,Q){
        ll q;cin >> q;
        if(q == 1){
            ...
            ll max = MT.query(0,N);
            ll sum = 0;
            auto dfs = [&](auto dfs, ll left, ll right) -> void {
                if (right == left + 1){
                    ll m;
                    if(r < A[left]){
                        m = r;
                ...
                if(r <= min){
                    ...
                    sum += ST.query(left, right);
                }
                else{
                    ll mid = (right+left)/2;
                    max = MT.query(left,mid);
                    min = MT.query(left,mid);
                    dfs(dfs, left, mid);
                    max = MT.query(mid,right);
                    min = MT.query(mid,right);
                    dfs(dfs, mid, right);
                }
            };
            if(l >= r){
