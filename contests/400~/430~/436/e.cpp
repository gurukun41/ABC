...
bool check(vl &X, ll now, ll count){
    if(count > mini){
        return false;
    }
    rep(i,now,X.size()){
        if(X[i] != i){
            pl temp1 = {i,X[i]};
            pl temp2 = {X[i],X[X[i]]};
            X[temp1.first] = temp2.second;
            X[temp2.first] = temp1.second;
            bool che = check(X, temp1.first, count+1);
            X[temp1.first] = temp1.second;
            X[temp2.first] = temp2.second;
            if(che){
                return true;
            } else {
                return false;
...
int main(){
    ...
    rep(i,0,N){
        rep(j,i+1,N){
            pl temp1 = {i,P[i]};
            pl temp2 = {j,P[j]};
            P[temp1.first] = temp2.second;
            P[temp2.first] = temp1.second;
            bool che = check(P, temp1.first, 1);
            P[temp1.first] = temp1.second;
            P[temp2.first] = temp2.second;
            if(che){
                ans++;
            }   
        }
