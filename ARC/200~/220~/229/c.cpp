...
int main(){
    ...
    rep(_,0,T){
        ...
        rep(i,0,A2.size()){
            sum += (A1[i]+A2[i])/2;
            if(i+1<A1.size()) sum += (A2[i]+A1[i+1])/2;
        }

        rep(i,A2.size(),A1.size()-1){
            sum += (A1[i]+A1[i+1])/2;
        }
        if(A1.size()==0){
            sum += (left+right)/2;
        ...
        if(left%2==A1[0]){
            sum += (right+A1[0])/2;
            if(A1.size() == A2.size()){
                sum += (left+A2[A2.size()-1])/2;
            }else{
                sum += (left+A1[A1.size()-1])/2;
            }
        } else {
            sum += (left+A1[0])/2;
            if(A1.size() == A2.size()){
                sum += (right+A2[A2.size()-1])/2;
            }else{
                sum += (right+A1[A1.size()-1])/2;
            }
        }
        ...
        rep(i,0,A2.size()){
            sum2 += (A1[i]+A2[i])/2;
            if(i+1<A1.size()) sum2 += (A2[i]+A1[i+1])/2;
        }
